...
public class DwsTradeSkuOrderWindowSyncCacheApp extends BaseApp {
    ...
    public void handle(StreamExecutionEnvironment env, DataStreamSource<String> stream) {
        ...
        //需要关联的表 级 关联顺序:dim_sku_info dim_spu_info dim_base_trademark dim_base_category3 dim_base_category2 dim_base_category1
         ...
         //Hbase获取数据 无Redis缓存
        // SingleOutputStreamOperator<TradeSkuOrderBean> joinDimStream = getStreamOperator(reducestream);

        // Hbase获取数据 Redis缓存 DimApp(数据改变删除缓存)  封装模版
        SingleOutputStreamOperator<TradeSkuOrderBean> joinDimStream = getStreamOperatorhuangchun(reducestream);
...
//        5. 写出到Doris
    ...
    private static SingleOutputStreamOperator<TradeSkuOrderBean> getStreamOperatorhuangchun(SingleOutputStreamOperator<TradeSkuOrderBean> reducestream) {

        // 关联 dim_sku_info
        return  reducestream.map(
    ...
    private static SingleOutputStreamOperator<TradeSkuOrderBean> getStreamOperator(SingleOutputStreamOperator<TradeSkuOrderBean> reducestream) {
        SingleOutputStreamOperator<TradeSkuOrderBean>  joinDimStream = reducestream.map(
